...
interface HomepageViewProps {
  initialConfig: HomepageFullConfig;
  isPreview?: boolean;
}

export function HomepageView({ initialConfig, isPreview = false }: HomepageViewProps) {
  const [homepageData, setHomepageData] = useState<any | null>(null);
  const [config, setConfig] = useState<HomepageFullConfig>(initialConfig);
  ...
  const fetchHomepageData = async () => {
    ...
    try {
      ...
      // Offline / fallback to initialConfig
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] text-slate-900 font-sans antialiased selection:bg-orange-500/20 selection:text-[#FF5500] relative">
      ...
      {/* ── MAIN HOMEPAGE SECTIONS (Directly Pre-Rendered on Server) ── */}
