...
export function EligibilityFormIsland() {
  const store = useEligibilityStore();

  // Input Form State
  const [sscGPA, setSscGPA] = useState<number | ''>(store.sscGPA);
  const [hscGPA, setHscGPA] = useState<number | ''>(store.hscGPA);
  const [group, setGroup] = useState<AcademicGroup>(store.group || 'Science');
  const [passingYear, setPassingYear] = useState<number>(store.passingYear || 2026);
  ...
  // Sync store values on hydration if available
  useEffect(() => {
    if (store.sscGPA !== '') setSscGPA(store.sscGPA);
    if (store.hscGPA !== '') setHscGPA(store.hscGPA);
    if (store.group) setGroup(store.group);
    if (store.passingYear) setPassingYear(store.passingYear);
  }, [store.sscGPA, store.hscGPA, store.group, store.passingYear]);
  ...
  const handleCheck = async () => {
    ...
    // Save to persisted Zustand store for cross-page persistence
    store.setScores({
    ...
    try {
      // Connect to live backend PostgreSQL eligibility check API
        ...
        <div className="bg-white/95 backdrop-blur-xl border border-orange-100/80 rounded-3xl p-6 sm:p-10 shadow-xl shadow-orange-500/5 space-y-8">
          {submitted && (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-bold text-[#FF5500] hover:text-[#E64D00] transition cursor-pointer px-3 py-1.5 rounded-full bg-orange-50 border border-orange-200"
            >
