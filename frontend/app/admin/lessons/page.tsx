...
export default function AdminLessonsPage() {
  ...
  const lessons = [
    {
      id: 'l1',
      title: "Newton's Second Law & Linear Momentum (BUET Standard)",
      subject: 'Physics',
      chapter: "Newton's Mechanics",
      slug: 'newtons-second-law-buet-guide',
      status: 'Published',
      views: '2.4k',
      updated: '2 days ago',
    },
    {
      id: 'l2',
      title: 'Work-Energy Theorem and Conservative Forces',
      subject: 'Physics',
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="p-5 rounded-3xl bg-white border border-slate-200/80 shadow-xs flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Published Lessons</p>
              <h3 className="text-2xl font-black text-slate-900 font-mono">128 Lessons</h3>
          ...
          <div className="p-5 rounded-3xl bg-white border border-slate-200/80 shadow-xs flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Chapters Covered</p>
          ...
          <div className="p-5 rounded-3xl bg-white border border-slate-200/80 shadow-xs flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Student Views</p>
              <h3 className="text-2xl font-black text-slate-900 font-mono">7.3k Views</h3>
              <p className="text-[11px] text-emerald-600 font-semibold flex items-center gap-1">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50/80 border-b border-slate-200/80 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                <tr>
                  <th className="py-3.5 px-5">Lesson Title</th>
                  <th className="py-3.5 px-4">Subject • Chapter</th>
                  <th className="py-3.5 px-4">Student Views</th>
                  ...
                  <tr key={l.id} className="hover:bg-slate-50/70 transition">
                    <td className="py-4 px-5 max-w-md">
                      ...
                      <div className="text-[11px] text-slate-400 font-mono mt-0.5">
                        /prepare/lessons/{l.slug}
                      </div>
                    </td>
                    <td className="py-4 px-4">
                      <div className="font-semibold text-slate-900 text-xs">{l.subject}</div>
                      <div className="text-[11px] text-slate-500">{l.chapter}</div>
                    </td>
                    <td className="py-4 px-4">
                      <span className="font-mono font-bold text-slate-900 text-xs">{l.views}</span>
                    </td>
                    <td className="py-4 px-4">
                    ...
                    <td className="py-4 px-4 text-slate-400 text-[11px] font-medium">{l.updated}</td>
                    <td className="py-4 px-5 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <Link
                          href={`/prepare/lessons/${l.slug}`}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm overflow-y-auto">
            <div className="bg-white border border-slate-200/90 rounded-3xl max-w-3xl w-full p-6 sm:p-7 space-y-5 shadow-2xl max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                <div>
                  <h3 className="font-black text-lg text-slate-900">Publish Interactive Lesson</h3>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Create structured curriculum chapters with rich formulas and concepts
                  </p>
                </div>
                <button
              ...
              <form onSubmit={handleCreate} className="space-y-4 text-xs">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-900 block">Lesson Title *</label>
                  <input
                    required
                    value={title}
                    onChange={(e) => setTitle(e.target.value)}
                    placeholder="e.g. Projectile Motion Max Height & Trajectory"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-900 block">Chapter Name</label>
                    <input
                      value={chapter}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-900 block">
                    Lesson Content (Quill Rich Text Editor)
                  </label>
                  <RichEditor
                    placeholder="Write lesson notes, formulas, and examples..."
                    onChange={setBody}
                  />
                </div>
